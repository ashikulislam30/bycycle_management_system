...
typedef struct RentalHistory
{
    ...
    int rental_duration;
    double rental_cost;
//    char rental_start_time[100];
//    char rental_end_time[100];
//    char pickup_station[100];
//    char return_station[100];
    char brand[100];
    char model[100];
    char username[100];
...
void showName(char *name);                              /// show menu name
void mainMenu();                                        /// main menu
void studentLogin();                                    /// student login checker
void studentSignup();
void studentIntermediateMenu();                         /// student login/signup menu
void studentMainMenu(char *name);                       /// student after login options
void adminLogin();                                      /// admin login checker
void adminMainMenu(char *name);                         /// admin after login options
void aboutDevelopers();                                 /// about developers
...
int main()
{
    ...
    insertAdminUser(adminUserList, "notthedroid", "notthedroid");
    insertAdminUser(adminUserList, "ashik01", "password");
    insertAdminUser(adminUserList, "ashik02", "1234");
...
void adminMainMenu(char *username)
{
    ...
    case 1:
    {
        ...
        int successUpdate = addBicycle(bicycleList, info);
    ...
    case 5:
    {
        ...
        viewRentalHistory(RentalHistoryList);
...
adminUserNode* createAdminUserNode(char *username, char *password)
{
...
void insertAdminUser(adminUserNode *head, char *username, char *password)
{
    adminUserNode *newNode = createAdminUserNode(username, password);
    if (head -> next != NULL)
    {
...
int addBicycle(bicycle* head, bicycleInfo info)
{
...
void addAdmin(adminUserNode *adminUserList, char *username)
{
    ...
    insertAdminUser(adminUserList, inputUsername, inputPassword);
...
void studentSignup(studentUserNode *studentUserList)
{
    ...
    /// opening file and storing studentUsername and password
//     studentFile = fopen("Student_Info.txt", "a");
//     if(studentFile == NULL)
//     {
//         printf("\n\t\t\t\tError Opening File!!\n");
//         return;
...
//
//     fprintf(studentFile, "\n%s", inputUsername);
//     fprintf(studentFile, " %s", inputPassword);
//
//     fclose(studentFile);

    insertStudentUser(studentUserList, inputUsername, inputPassword);
...
studentUserNode* createStudentUserNode(char *username, char *password)
{
    char hash_passowrd[500];
    strcpy(hash_passowrd,password);
    hashPassword(hash_passowrd);
...
void insertStudentUser(studentUserNode *head, char *username, char *password)
{
    studentUserNode *newNode = createStudentUserNode(username, password);
    if (head -> next != NULL)
    {
        newNode -> next = head -> next;
        head -> next = newNode;
...
int paymentForRent(bicycle *current, double rental_cost){
    ...
    switch(payment_choice){
        ...
        case 3:{
            char coming_soon[]="Feature coming soon.....";
            printf("\n\t\t\t\t");
...
void pay_with_bkash_or_nagad(int payment_choice, double rental_cost){
...
void viewRentalHistory(RentalHistoryNode *RentalHistoryList){
...
void aboutDevelopers()
{
    ...
    {
        { 1,"Md.Ashikul Islam", "213-15-14787", "60_A",}
    };
